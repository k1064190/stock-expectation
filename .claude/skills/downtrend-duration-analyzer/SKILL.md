...
- Ticker list for the universe to analyze

## Workflow

### Step 1: Fetch Historical Price Data

Fetch price history for each ticker using `bin/stock-cli`, adjusting `--days` to cover the desired lookback:

```bash
# US market — single ticker, 5-year lookback (~1260 trading days)
bin/stock-cli price AAPL --market US --days 1260

# KR market — single ticker
bin/stock-cli price 005930 --market KR --days 1260
```

Repeat for each ticker in the universe, then feed the collected OHLCV JSON to the analysis steps below. The original script `scripts/analyze_downtrends.py` is preserved for reference.
...
```bash
uv run python .claude/skills/downtrend-duration-analyzer/scripts/generate_histogram_html.py \
