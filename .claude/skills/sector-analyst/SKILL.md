...
## Market-Specific Sector Proxies
...
### KR Market
005930 Samsung (Tech), 105560 KB Financial (Financials), 096770 SK Innovation (Energy),
207940 Samsung Biologics (Healthcare), 010130 Korea Zinc (Materials),
051910 LG Chem (Chemicals), 035420 NAVER (Communication), 034730 SK (Conglomerate)

## Data Fetching

Fetch price data for the appropriate sector proxy basket using `bin/stock-cli`:

```bash
# US sector ETFs
bin/stock-cli price-batch XLK,XLF,XLE,XLV,XLI,XLP,XLY,XLU,XLRE,XLB,XLC --market US --days 90

# KR sector proxies
bin/stock-cli price-batch 005930,105560,096770,207940,010130,051910,035420,034730 --market KR --days 90
```

The original script `scripts/analyze_sector_rotation.py` (TraderMonty CSV fetch) is preserved for reference.
...
1. Run the appropriate `bin/stock-cli price-batch` command above
