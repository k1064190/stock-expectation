...
```bash
uv run python .claude/skills/canslim-screener/scripts/screen_canslim.py
```
...
### Step 3: Fetch Data and Execute CANSLIM Screening

Fetch fundamentals and price data for the candidate universe using `bin/stock-cli`:

```bash
# Fetch fundamentals (for C and A components: earnings, growth rates)
bin/stock-cli fundamentals-batch AAPL,MSFT,NVDA,AMZN,META --market US

# Fetch 365 days of price data (for N, S, L, M components)
bin/stock-cli price-batch AAPL,MSFT,NVDA,AMZN,META --market US --days 365

# KR market
bin/stock-cli fundamentals-batch 005930,000660,035420,051910 --market KR
bin/stock-cli price-batch 005930,000660,035420,051910 --market KR --days 365
```

> **Note:** KR earnings growth data (C and A components) may be limited — yfinance/PyKRX quarterly earnings coverage is less complete than FMP for US stocks.

The original script `scripts/screen_canslim.py` is preserved for reference (FMP API required for full automated run).

**Analysis workflow using fetched data (Phase 3 - Full CANSLIM):**
1. **Market Direction (M)**: Assess S&P 500 (or KOSPI for KR) trend vs 50-day EMA from price-batch output
   - If bear market detected (M=0), warn user to raise cash
2. **Stock Analysis**: For each ticker, calculate:
   - **C Component**: Quarterly EPS/revenue growth (YoY) from fundamentals-batch
   - **A Component**: 3-year EPS CAGR and stability from fundamentals-batch
   - **N Component**: Distance from 52-week high, breakout detection from price-batch
   - **S Component**: Volume-based accumulation/distribution (up-day vs down-day volume) from price-batch
   - **L Component**: 52-week Relative Strength vs benchmark from price-batch
   - **I Component**: Institutional holder count + ownership % (use WebSearch for KR if needed)
3. **Composite Scoring**: Weighted average with all 7 component breakdown
