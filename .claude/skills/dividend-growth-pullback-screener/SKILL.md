...
### Step 1: Fetch Fundamental and Price Data

Use `bin/stock-cli` to gather fundamentals and price history for the candidate ticker list:

```bash
# Fetch fundamentals for a US candidate list
bin/stock-cli fundamentals-batch MSFT,V,MA,AAPL,HD --market US

# Fetch fundamentals for a KR candidate list
...
```bash
# Fetch 90 days of price data (for 14-period RSI calculation)
bin/stock-cli price-batch MSFT,V,MA,AAPL,HD --market US --days 90
bin/stock-cli price-batch 005930,017670,030200 --market KR --days 90
```

The original FMP/FINVIZ screening script is preserved in `scripts/screen_dividend_growth_rsi.py` for reference.

### Step 2: Apply Screening Criteria

Using the fetched data:
1. FINVIZ/fundamental pre-filter: Dividend yield 0.5–3%, dividend growth 10%+, EPS growth 5%+, sales growth 5%+
2. Verify 12%+ dividend CAGR from fundamentals-batch output
3. Calculate 14-period RSI from price-batch output; require RSI <= 40
4. Assess financial health: D/E < 2, current ratio > 1, positive revenue and EPS growth
5. Apply composite scoring and ranking
