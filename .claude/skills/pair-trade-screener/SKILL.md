...
**Data Fetching via bin/stock-cli:**
```bash
# US market — fetch 365 days for a candidate pair
bin/stock-cli price-batch AAPL,MSFT --market US --days 365

# KR market — fetch a candidate pair
bin/stock-cli price-batch 005930,000660 --market KR --days 365
```

The original script `scripts/fetch_price_data.py` is preserved for reference.
...
**Purpose:** Screen for cointegrated pairs within a sector or custom list (reference implementation using FMP API directly).

**Preferred data fetching via bin/stock-cli:**
```bash
# Fetch price history for a set of tickers, then run statistical analysis
bin/stock-cli price-batch AAPL,MSFT,GOOGL,META --market US --days 365
bin/stock-cli price-batch 005930,000660,035720,051910 --market KR --days 365
```

**Legacy script usage (FMP API required):**
```bash
