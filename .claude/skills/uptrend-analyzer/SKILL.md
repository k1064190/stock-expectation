...
Use these representative baskets when Monty's CSV is unavailable or for KR analysis:
...
```bash
# US basket — fetch price data for breadth computation
...
> **Note:** Computed breadth from this basket is less precise than Monty's CSV (which covers ~2,800 stocks) but enables dual-market analysis. Expected runtime ~5 seconds.

The original script `scripts/uptrend_analyzer.py` (Monty CSV fetch, no API key) is preserved for US-only analysis when Monty's data is available.

Compute breadth from the fetched data:
1. For each ticker, determine whether price is above its 200-day MA (uptrend) or below (downtrend)
2. Calculate uptrend ratio = (stocks in uptrend) / (total stocks)
3. Feed into the 5-component scoring system
