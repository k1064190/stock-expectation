...
Screen stocks for Mark Minervini's Volatility Contraction Pattern (VCP), identifying Stage 2 uptrend stocks with contracting volatility near breakout pivot points.
...
- `bin/stock-cli` available in the project root (no separate API key needed)
- Provide a ticker list; any US or KR tickers are accepted

## Workflow

### Step 1: Fetch Price Data

Fetch 250 days of OHLCV data for the ticker universe using `bin/stock-cli`:

```bash
# US market — custom ticker list
bin/stock-cli price-batch AAPL,NVDA,MSFT,AMZN,META --market US --days 250

# KR market
bin/stock-cli price-batch 005930,000660,035420 --market KR --days 250
```

Pass the resulting JSON to the analysis steps below. The original FMP script is preserved in `scripts/screen_vcp.py` for reference and advanced parameter tuning.
...
```bash
uv run python .claude/skills/vcp-screener/scripts/screen_vcp.py \
  --min-contractions 3 \
  --t1-depth-min 12.0 \
